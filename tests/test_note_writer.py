...
class TestDetectNoteIntentPositives:
    ...
    def test_note_que(self):
        assert mn._detect_note_intent("note que je préfère vim") is True

    def test_notes_que(self):
        assert mn._detect_note_intent("notes que j'habite à Bordeaux") is True

    def test_retiens_que(self):
        assert mn._detect_note_intent("retiens que mon projet s'appelle Mnemo") is True

    def test_retiens_bien_que(self):
        assert mn._detect_note_intent("retiens bien que je suis allergique au gluten") is True

    def test_memorise_que(self):
        assert mn._detect_note_intent("mémorise que j'utilise FastAPI") is True

    def test_memorise_ca(self):
        assert mn._detect_note_intent("mémorise ça") is True

    def test_memorise_ceci(self):
        assert mn._detect_note_intent("mémorise ceci : je préfère Python") is True

    def test_noublie_pas_que(self):
        assert mn._detect_note_intent("n'oublie pas que je travaille en remote") is True

    def test_noublie_pas_ca(self):
        assert mn._detect_note_intent("n'oublie pas ça") is True

    def test_souviens_toi_que(self):
        assert mn._detect_note_intent("souviens-toi que j'ai un chat") is True

    def test_souviens_toi_que_sans_tiret(self):
        assert mn._detect_note_intent("souviens toi que je suis développeur") is True

    def test_garde_en_memoire(self):
        assert mn._detect_note_intent("garde en mémoire que je préfère le thé") is True

    def test_garde_ca_en_memoire(self):
        assert mn._detect_note_intent("garde ça en mémoire") is True

    def test_enregistre_que(self):
        assert mn._detect_note_intent("enregistre que mon budget est 500€") is True

    def test_enregistre_ceci(self):
        assert mn._detect_note_intent("enregistre ceci") is True

    def test_enregistre_ca(self):
        assert mn._detect_note_intent("enregistre ça") is True

    def test_ajoute_a_ma_memoire(self):
        assert mn._detect_note_intent("ajoute à ma mémoire que je déteste les réunions") is True

    def test_ecris_dans_ma_memoire_sans_accent(self):
        assert mn._detect_note_intent("ecris dans ma memoire que j'aime le vélo") is True

    def test_ecris_dans_ma_memoire_avec_accent(self):
        assert mn._detect_note_intent("écris dans ma mémoire ma décision") is True

    def test_ajoute_a_mes_notes(self):
        assert mn._detect_note_intent("ajoute à mes notes que j'utilise uv") is True

    def test_ajoute_dans_mes_notes(self):
        assert mn._detect_note_intent("ajoute dans mes notes : deadline vendredi") is True

    def test_important_a_noter_avec_accent(self):
        assert mn._detect_note_intent("important à noter : réunion annulée") is True

    def test_important_a_noter_sans_accent(self):
        assert mn._detect_note_intent("important a noter que le client a changé") is True

    def test_a_noter_avec_accent(self):
        assert mn._detect_note_intent("à noter : livraison demain") is True

    def test_a_noter_sans_accent(self):
        assert mn._detect_note_intent("a noter : nouvelle adresse mail") is True

    def test_case_insensitive(self):
        assert mn._detect_note_intent("NOTE QUE je préfère le café") is True

    def test_mixed_case(self):
        assert mn._detect_note_intent("Retiens Que mon pseudo c'est Mathi") is True

    def test_keyword_in_middle_of_sentence(self):
        assert mn._detect_note_intent("s'il te plaît note que je suis gaucher") is True
...
class TestDetectNoteIntentNegatives:
    ...
    def test_simple_question(self):
        assert mn._detect_note_intent("comment tu vas ?") is False

    def test_conversation_greeting(self):
        assert mn._detect_note_intent("bonjour Mnemo") is False

    def test_memory_query(self):
        assert mn._detect_note_intent("qu'est-ce que je t'ai dit sur mon projet ?") is False

    def test_calendar_query(self):
        assert mn._detect_note_intent("qu'est-ce que j'ai prévu demain ?") is False

    def test_shell_command(self):
        assert mn._detect_note_intent("liste les fichiers dans docs") is False

    def test_scheduler_request(self):
        assert mn._detect_note_intent("rappelle-moi dans 3h de prendre mes médicaments") is False

    def test_philosophical_note(self):
        # "note" seul en milieu de phrase technique ne doit pas déclencher
        assert mn._detect_note_intent("prends note mentalement") is False

    def test_technical_context_note(self):
        # "note" dans contexte de musique ne doit pas déclencher
        assert mn._detect_note_intent("quelle est la note de ce morceau ?") is False

    def test_empty_string(self):
        assert mn._detect_note_intent("") is False

    def test_merci(self):
        assert mn._detect_note_intent("merci !") is False

    def test_souvenir_without_keyword(self):
        # "souviens" seul sans "toi que" ne doit pas déclencher
        assert mn._detect_note_intent("tu t'en souviens ?") is False
...
# ══════════════════════════════════════════════════════════════════
# 3. _route_message — dispatch route=note
# ══════════════════════════════════════════════════════════════════

class TestRouteNote:

    def _call_route_note(self, note_response="Note sauvegardée."):
        eval_json = _eval_json_note()
        with patch.object(mn, 'NoteWriterCrew') as mock_note_cls, \
             patch.object(mn, 'ConversationCrew') as mock_conv_cls:
            mock_note_cls.return_value.run.return_value = note_response
            result = mn._route_message(eval_json, "note que je préfère vim",
                                       "s1", "ctx", "")
    ...
    def test_note_route_passes_user_message(self):
        ...
        with patch.object(mn, 'NoteWriterCrew') as mock_note_cls, \
            ...
            mock_conv_cls.return_value.crew.return_value.kickoff.return_value = _mock_crew_result("ok")
            mn._route_message(eval_json, user_msg, "s1", "ctx", "")
    ...
    def test_note_route_distinct_from_conversation(self):
        ...
        conv_json = _eval_json_note(route="conversation")
        with patch.object(mn, 'NoteWriterCrew') as mock_note, \
             patch.object(mn, 'ConversationCrew') as mock_conv:
            mock_conv.return_value.crew.return_value.kickoff.return_value = _mock_crew_result("réponse conv")
            mn._route_message(conv_json, "test", "s1", "ctx", "")
...
class TestHandleMessageNoteBypass:
    ...
    def test_note_keyword_bypasses_evaluation_crew(self):
        """Quand kw_note=True, EvaluationCrew ne doit pas être appelé."""
        with patch.object(mn, 'EvaluationCrew') as mock_eval, \
             patch.object(mn, 'NoteWriterCrew') as mock_note, \
             patch.object(mn, 'get_temporal_context', return_value="ctx"), \
             patch.object(mn, 'update_session_memory'):
            mock_note.return_value.run.return_value = "Note enregistrée."
    ...
    def test_note_keyword_returns_note_crew_response(self):
        with patch.object(mn, 'EvaluationCrew'), \
             patch.object(mn, 'NoteWriterCrew') as mock_note, \
             patch.object(mn, 'get_temporal_context', return_value="ctx"), \
             patch.object(mn, 'update_session_memory'):
            mock_note.return_value.run.return_value = "Bordeaux enregistré."
            result = mn.handle_message("note que j'habite à Bordeaux", "session_1")
        assert result == "Bordeaux enregistré."

    def test_non_note_message_does_not_bypass_to_note_crew(self):
        """Un message sans keyword note doit passer par EvaluationCrew."""
        eval_json = _eval_json_note(route="conversation")
        eval_mock = MagicMock()
        eval_mock.raw = json.dumps(eval_json)
        # On force le ML à retourner une confiance basse pour éviter le bypass should_skip_llm
        with patch.object(mn, 'EvaluationCrew') as mock_eval, \
             patch.object(mn, 'NoteWriterCrew') as mock_note, \
             patch.object(mn, 'ConversationCrew') as mock_conv, \
             patch.object(mn, 'get_temporal_context', return_value="ctx"), \
             patch.object(mn, 'update_session_memory'), \
             patch.object(mn, '_ml_detect_intent', return_value=("conversation", 0.5)):
            mock_eval.return_value.crew.return_value.kickoff.return_value = eval_mock
            mock_conv.return_value.crew.return_value.kickoff.return_value = _mock_crew_result("réponse")
            mn.handle_message("comment tu vas ?", "session_1")
        assert mock_eval.called
        assert not mock_note.called

    def test_note_route_in_all_routes_loop(self):
        """route=note dispatche sans exception dans _route_message."""
        eval_json = _eval_json_note()
        with patch.object(mn, 'NoteWriterCrew') as mock_note:
            mock_note.return_value.run.return_value = "ok"
            result = mn._route_message(eval_json, "msg", "s1", "ctx", "")
        assert result == "ok"
